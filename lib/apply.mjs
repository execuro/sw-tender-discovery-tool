...
export function apply({ root, doc, report }) {
  ...
  const newCqIds = [];
  const questions = model.questions.slice();
  ...
  for (const ri of report.items || []) {
    ...
    if (ri.blockedBy) {
      ...
      const cqId = isNewCq ? newCqIds[Number(NEW_CQ_RE.exec(ri.blockedBy)[1])] : ri.blockedBy;
      ...
      const existingQ = !isNewCq && questions.find(q => q.id === cqId);
      const sameKnownBlock = existingQ && existingQ.items.includes(item.id);
      if ((wasConfirmed || wasReopened) && sameKnownBlock && !substantive) {
