...
export class Session {
  ...
  readProfileHere() { return readProfile(this.root); }
  ...
  applyDecisions(decisions) {
    ...
    const writes = decisions.map(d => ({ type: d.action === 'reject' ? 'proposalReject' : 'proposalAccept', id: d.proposal }));
    if (this.lock) {
    ...
    for (const op of writes) {
      try { const r = this.applyOpWrite(op); if (r.ok) applied++; }
      catch (e) { this.log('decision apply failed', e.message); }
    }
  ...
  enqueue(body) {
    ...
    // SI-4/IN-11: an analyze batch's `items` — up to ~15 ids, in page order, that still need
    ...
    if (kind === 'analyze') {
      batch.items = Array.isArray(body.items) && body.items.length
        ? body.items.slice()
        : nextAnalyzeItems(this.model?.items, proposals.reestimateList(this.root, this.slug));
    }
