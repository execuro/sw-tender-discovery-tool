...
/** The next chunk for a progressive analyze batch (IN-11): up to `limit` item ids, in page
 * order (`pageOrder`, above), that still need work — `queued`/`reopened`, or named by
 * `reestimate.json` (an `item: "*"` mark means every item). `reestimate` is the parsed
 * `reestimate.json` array (or `[]`), same shape `proposals.reestimateList` returns. */
export function nextAnalyzeItems(items, reestimate = [], limit = 15) {
  const marks = reestimate || [];
  const star = marks.some(r => r.item === '*');
  const marked = new Set(marks.map(r => r.item));
  const due = pageOrder(items).filter(it => star || NEEDS_ANALYZE_STATUS.has(it.status?.kind) || marked.has(it.id));
