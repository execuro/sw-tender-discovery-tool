  ...
  try {
    while (Date.now() < deadline) {
      ...
      if (body.event === 'batch') {
        ...
        // SI-4/IN-11: an analyze batch's `items` (up to ~15 ids, page order) — the ONLY items to
        // assess this chunk; the rest of the document is not in scope for this batch.
        if (Array.isArray(body.batch?.items) && body.batch.items.length) out.line('items', body.batch.items.join(', '));
