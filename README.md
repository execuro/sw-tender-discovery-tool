...
Local, zero-dependency runtime that renders a tender analysis sidecar (`specs/rfp-NNNN-slug-analysis.md`, written by `sw-discover-tender`) as one live page. The operator confirms scope items, answers client questions (queued until Send), edits the Client Response and Internal note in place, and runs the partner-profile wizard; the runtime applies every one of those directly. Accepting or rejecting a proposed assumption, or picking an option on a client question, instead queues a decision alongside notes, applied in one batch (with the queue's own auto-send once decisions pile up) rather than one call per click. Only a batch (`intake`, `analyze`, `reestimate`, `notes`, `export`) spawns an agent. Annotations, chat and decisions collect in the Agent card's collapsible **Queued (n)** block; the agent panel itself folds to a slim rail so the working sheets get more room. Spawned by `sw-discover-tender … --editor`.
...
| `lib/profile.mjs` | the partner profile (`var/sw-ai-sdk/tender/partner-profile.md` — one per project checkout, shared by every tender, gitignored through Shopware's `/var/*`; a pre-0.2.0 `specs/rfp-partner-profile.md` is still read until the next save) and the T-shirt/profile regime |
...
| `page/app.js` chat | each batch bubble shows its live state (`working` with stage and elapsed time from the `run` event's `startedAt`, `queued`, `done`, `aborted`) |
...
| Tab | Shows |
| --- | --- |
| Overview | §2 totals, confidence, the export gate, every question no requirement row owns; a Not-taken banner shows whenever there are Not-taken rows or unused sheets — item counts per tab, the rows (each with a Restore button), unused sheets, answer targets per client table; intake confirms itself and starts the analysis automatically, in chunks of up to 45 items (IN-3/IN-11) — a header pill shows "Analysing n of N" while a chain is in flight |
...
| `GET`/`PUT /api/profile` | read / write `var/sw-ai-sdk/tender/partner-profile.md` (`GET` also returns `path`, and `legacy: true` while an old `specs/rfp-partner-profile.md` is the one being read); a `PUT` marks every item for re-estimate |
